...
class LVariance(tf.keras.regularizers.Regularizer):
    def __init__(self, lvariance=0.0):
...
class L1Variance(tf.keras.regularizers.Regularizer):
    def __init__(self, l1=0.0, lvariance=0.0):
        self.l1 = l1
        self.lvariance = lvariance

    def __call__(self, x):
        return self.l1 * tf.math.reduce_sum(
            tf.math.abs(x)
        ) + self.lvariance * tf.math.reduce_sum(tf.math.reduce_variance(x, axis=0))
...
class LRange(tf.keras.regularizers.Regularizer):
    def __init__(self, lrange=0.0, limit=0.05):
        self.lrange = lrange
        self.limit = limit
    ...
    def __call__(self, x):
        ...
        return self.lrange * tf.math.reduce_sum(
            tf.math.reduce_variance(out_bounary_variance, axis=0)
        )
...
class L1_L2(tf.keras.regularizers.Regularizer):
    def __init__(self, l1_l2=0.0):
        self.l1_l2 = l1_l2

    def __call__(self, x):
        return self.l1_l2 * tf.math.reduce_sum(
            tf.math.square(tf.math.reduce_sum(tf.math.abs(x), axis=1))
        )
...
class LEntropy(tf.keras.regularizers.Regularizer):
    def __init__(self, lentropy=0.0, alpha=0.0):
